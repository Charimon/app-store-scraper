...
function extractReviews (xml) {
  const toJSON = (item) => {
    return {
      ...
      date: (new Date(item.updated && item.updated[0])),
      userName: item.author && item.author[0] && item.author[0].name && item.author[0].name[0],
      userUrl: item.author && item.author[0] && item.author[0].uri && item.author[0].uri[0],
      version: item['im:version'] && item['im:version'][0],
      score: parseInt(item['im:rating'] && item['im:rating'][0]),
      title: item.title && item.title[0],
      text: item.content && item.content[0] && item.content[0]['_'],
      url: item.link && item.link[0] && item.link[0].$ && item.link[0].$.href
    };
  };
